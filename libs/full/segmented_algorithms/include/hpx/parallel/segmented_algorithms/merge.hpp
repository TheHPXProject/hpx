...
namespace hpx::parallel::detail {
        ...
        hpx::future<OutLocIterator>, OutLocIterator>
    {
        ...
        try
        {
            first_intersection = segmented_diagonal_intersection(first1, len1,
                first2, len2, k0, comparator, projection1, projection2);

            last_intersection = segmented_diagonal_intersection(first1, len1,
                first2, len2, k1, comparator, projection1, projection2);
        }
        catch (...)
        {
            ...
            if constexpr (Async)
            {
                ...
                return make_policy_exceptional_future<policy_type,
                    OutLocIterator>(exception);
            }
            else
            {
