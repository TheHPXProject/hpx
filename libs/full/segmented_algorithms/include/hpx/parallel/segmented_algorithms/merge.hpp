...
namespace hpx::parallel::detail {
    ...
    std::pair<std::size_t, std::size_t> segmented_diagonal_intersection(Iter1 first1,
    ...
    HPX_FORCEINLINE auto for_each_output_chunk(Traits3, Iter3 const& dest, 
        std::size_t total_size, F&& handle_chunk) 
        ...
        typename Traits3::local_iterator>
    {
        ...
        {
            out_local_iterator_type partition_end = 
                Traits3::end(seg_out);
    ...
    HPX_FORCEINLINE auto process_chunk(Traits3, Algo&& algo, ExPolicy& policy, IsSeq, 
        Iter1 const& first1, std::size_t len1, Iter2 const& first2, std::size_t len2, 
        SegIteratorOut seg_out, OutLocIterator out_first, std::size_t k0, std::size_t k1, 
        Comp& comparator, Proj1& projection1, Proj2& projection2) -> std::conditional_t<Async, hpx::future<OutLocIterator>, OutLocIterator>
    {
        using value_type1 = 
        ...
        auto [a0, b0] = segmented_diagonal_intersection(first1, len1, first2, len2, k0, 
            comparator, projection1, projection2);
        auto [a1, b1] = segmented_diagonal_intersection(first1, len1, first2, len2, k1,
            comparator, projection1, projection2);
        ...
        if (chunk_first1 == chunk_last1)
        {
            if constexpr (Async)
            {
                return capture_copy_async<value_type2>(Traits3{}, policy, IsSeq{}, 
                    seg_out, chunk_first2, chunk_last2, HPX_MOVE(out_first));
            }
            else
            {
                return capture_copy<value_type2>(Traits3{}, policy, IsSeq{}, seg_out, 
                    chunk_first2, chunk_last2, HPX_MOVE(out_first));
            }
        }
        ...
        if (chunk_first2 == chunk_last2)
        {
            if constexpr (Async)
            {
                return capture_copy_async<value_type1>(Traits3{}, policy, IsSeq{}, 
                    seg_out, chunk_first1, chunk_last1, HPX_MOVE(out_first));
            }
            else
            {
                return capture_copy<value_type1>(Traits3{}, policy, IsSeq{}, seg_out, 
                    chunk_first1, chunk_last1, HPX_MOVE(out_first));
            }
        }

        if constexpr (Async)
        {
            return capture_dispatch_async(Traits3{}, algo, policy, IsSeq{}, seg_out, 
    ...
    HPX_FORCEINLINE util::detail::algorithm_result_t<ExPolicy, util::in_in_out_result<Iter1, Iter2, Iter3>>
        segmented_merge_sequential(Algo&& algo, ExPolicy policy, Iter1 first1, Iter1 last1, Iter2 first2, 
        Iter2 last2, Iter3 dest, Comp&& comp, Proj1&& proj1, Proj2&& proj2, std::size_t len1, std::size_t len2)
    {
        using traits3 = 
            hpx::traits::segmented_iterator_traits<Iter3>;
        using segment_iterator_out = 
            typename traits3::segment_iterator;
        using out_local_iterator_type = 
            typename traits3::local_iterator;
        ...
        if constexpr (is_task_policy)
        {
            ...
            auto state = std::make_shared<task_state>(task_state{
                HPX_FORWARD(Algo, algo), HPX_MOVE(policy), 
                HPX_FORWARD(Comp, comp), HPX_FORWARD(Proj1, proj1), 
                HPX_FORWARD(Proj2, proj2), HPX_MOVE(first1), len1, 
                HPX_MOVE(first2), len2});
            ...
            auto output_position = 
                ...
                std::size_t k1){
                    auto chunk_operation = [state, seg_out, out_first, 
                        k0, k1] () mutable {
                    ...
                    else
                    {
                        sequential_operation = sequential_operation.then(
                            [chunk_operation = HPX_MOVE(chunk_operation)](
                                auto previous) mutable -> hpx::future<out_local_iterator_type> {
                                    previous.get();
            ...
            auto end_dest_future = sequential_operation.then(
                [final_segment = HPX_MOVE(output_position.first)](
                    auto ready) mutable -> Iter3 {
                        ...
                        return traits3::compose(final_segment, HPX_MOVE(final_local));
                    });
        ...
        {
            auto output_position = 
                for_each_output_chunk(traits3{}, dest, total_size, [&](segment_iterator_out seg_out, 
                    out_local_iterator_type& out_first, std::size_t k0, std::size_t k1){
                        out_first = 
                            process_chunk<false>(traits3{}, algo, policy, std::true_type{}, 
                                first1, len1, first2, len2, seg_out, out_first, k0, k1, 
                                comp, proj1, proj2);
                    });
            
            Iter3 end_dest = traits3::compose(output_position.first, HPX_MOVE(output_position.second));

            return result::get(result_value_type{HPX_MOVE(last1), HPX_MOVE(last2), HPX_MOVE(end_dest)});
        }
    }
    ...
    template <typename Algo, typename ExPolicy, typename Iter1, typename Iter2, typename Iter3, 
        typename Comp, typename Proj1, typename Proj2>
    HPX_FORCEINLINE util::detail::algorithm_result_t<ExPolicy, util::in_in_out_result<Iter1, Iter2, Iter3>>
        segmented_merge_parallel(Algo&& algo, ExPolicy policy, Iter1 first1, Iter1 last1, Iter2 first2, 
        Iter2 last2, Iter3 dest, Comp&& comp, Proj1&& proj1, Proj2&& proj2, std::size_t len1, std::size_t len2)
    {
        using traits3 = 
            hpx::traits::segmented_iterator_traits<Iter3>;
            ...
            for_each_output_chunk(traits3{}, dest, total_size, 
                [&](segment_iterator_out seg_out, out_local_iterator_type out_first, 
                    std::size_t k0, std::size_t k1){
                        parallel_operations.push_back(
                            process_chunk<true>(traits3{}, algo, policy, std::false_type{}, 
                                first1, len1, first2, len2, seg_out, out_first, k0, k1, 
                                comp, proj1, proj2));
                    });
        ...
        auto end_dest_future = 
                    hpx::when_all(HPX_MOVE(parallel_operations)).then([
                        final_segment = HPX_MOVE(output_position.first)](
                            auto ready) mutable -> Iter3 {
        ...
        auto operation = end_dest_future.then([last1 = HPX_MOVE(last1), last2 = HPX_MOVE(last2)](
            auto ready) mutable -> result_value_type {
                return result_value_type{
                    HPX_MOVE(last1), HPX_MOVE(last2), ready.get()};
            });
    ...
    template <typename Algo, typename ExPolicy, typename Iter1, typename Iter2, typename Iter3, 
        typename Comp, typename Proj1, typename Proj2>
    util::detail::algorithm_result_t<ExPolicy, util::in_in_out_result<Iter1, Iter2, Iter3>>
        segmented_merge(Algo&& algo, ExPolicy policy, Iter1 first1, Iter1 last1, Iter2 first2, 
        Iter2 last2, Iter3 dest, Comp&& comp, Proj1&& proj1, Proj2&& proj2)
    {
        using policy_type = std::decay_t<ExPolicy>;
        using is_seq = 
            hpx::is_sequenced_execution_policy<policy_type>;
        using result_value_type = 
            util::in_in_out_result<Iter1, Iter2, Iter3>;
        using result = 
            util::detail::algorithm_result<ExPolicy, result_value_type>;
...
namespace hpx::segmented {
    ...
    hpx::parallel::util::detail::algorithm_result_t<ExPolicy, Iter3> 
    tag_invoke(hpx::merge_t, ExPolicy&& policy, Iter1 first1, Iter1 last1, 
        Iter2 first2, Iter2 last2, Iter3 dest, Comp comp = Comp())
    {
        ...
        using traits1 =
            hpx::traits::segmented_iterator_traits<Iter1>;
        using traits2 =
            hpx::traits::segmented_iterator_traits<Iter2>;
        using traits3 =
            hpx::traits::segmented_iterator_traits<Iter3>;
        ...
        return hpx::parallel::util::get_third_element(
            hpx::parallel::detail::segmented_merge(
                hpx::parallel::detail::merge<local_result>{},
                HPX_FORWARD(ExPolicy, policy), first1, last1,
    ...
    HPX_CXX_EXPORT template <typename Iter1, typename Iter2,
        ...
        hpx::traits::is_iterator_v<Iter3> && 
        hpx::traits::is_segmented_iterator_v<Iter3>)
    Iter3 tag_invoke(hpx::merge_t, Iter1 first1, Iter1 last1, 
        Iter2 first2, Iter2 last2, Iter3 dest, Comp comp = Comp())
    {
        static_assert(std::random_access_iterator<Iter1>, 
            "Requires a random-access iterator");
    ...
    tag_invoke(hpx::ranges::merge_t, ExPolicy&& policy,
