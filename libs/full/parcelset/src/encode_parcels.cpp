...
namespace hpx::parcelset {
    ...
    namespace detail {
        ...
        ///////////////////////////////////////////////////////////////////
        constexpr char to_digit(int const number) noexcept
        {
        ...
        constexpr void convert_byte(
            std::uint8_t const b, char* buffer, char const* /* end */) noexcept
        {
            *buffer++ = to_digit((b & 0xF0) >> 4);
            *buffer = to_digit(b & 0x0F);
        }

        std::string binary_archive_content(parcel_buffer const& buffer)
        {
            std::string result;
        ...
        void encode_finalize(parcel_buffer& buffer, std::size_t const arg_size)
        {
            ...
            using transmission_chunk_type =
                parcel_buffer::transmission_chunk_type;
            using count_chunks_type = parcel_buffer::count_chunks_type;
    ...
    std::size_t encode_parcels(parcelport& pp, parcel const* ps,
        std::size_t const num_parcels, parcel_buffer& buffer,
        std::uint32_t archive_flags, std::uint64_t const max_outbound_size)
    {
