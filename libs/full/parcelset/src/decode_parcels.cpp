...
namespace hpx::parcelset {
    ...
    std::vector<serialization::serialization_chunk> decode_chunks(
        parcel_buffer& buffer)
    {
        using transmission_chunk_type = parcel_buffer::transmission_chunk_type;
        ...
        if (num_zero_copy_chunks != 0)
        {
            ...
            {
                transmission_chunk_type const& c =
                    buffer.transmission_chunks_[i];
        ...
        parcel_buffer& buffer)
    {
        ...
        if (num_zero_copy_chunks != 0)
        {
            ...
            for (std::size_t i = 0; i != num_zero_copy_chunks; ++i)
            {
                ...
                auto const first = static_cast<std::size_t>(
                    static_cast<std::uint64_t>(c.first));
                [[maybe_unused]] auto const second = static_cast<std::size_t>(
                    static_cast<std::uint64_t>(c.second));
    ...
    std::vector<parcelset::parcel> decode_message_with_chunks(
        ...
        [[maybe_unused]] add_received_data_cb&& add_received_data,
        [[maybe_unused]] parcel_buffer& buffer, std::size_t parcel_count,
    ...
    defined(HPX_HAVE_PARCELPORT_ACTION_COUNTERS)
                    ...
                    add_received_data(p.get_action_name(), action_data);
#endif
                ...
                add_received_data(nullptr, data);
#endif
        ...
        std::size_t const num_thread)
    {
        ...
        return decode_message_with_chunks(archive, HPX_MOVE(add_received_data),
            buffer, parcel_count, num_thread);
    }

    std::vector<parcelset::parcel> decode_message(
        add_received_data_cb&& add_received_data, parcel_buffer buffer,
        std::size_t const parcel_count, std::size_t const num_thread)
    {
        std::vector<serialization::serialization_chunk> chunks(
            decode_chunks(buffer));
        return decode_message_with_chunks(HPX_MOVE(add_received_data),
            HPX_MOVE(buffer), parcel_count, chunks, num_thread);
    }

    ///////////////////////////////////////////////////////////////////////////
    std::vector<parcelset::parcel> decode_message_with_chunks_zero_copy(
        [[maybe_unused]] add_received_data_cb&& add_received_data,
        parcel_buffer& buffer, std::size_t const parcel_count,
        std::vector<serialization::serialization_chunk>& chunks,
        std::size_t const num_thread)
    {
        auto const inbound_data_size = static_cast<std::size_t>(
    ...
    std::vector<parcelset::parcel> decode_message_zero_copy(
        add_received_data_cb&& add_received_data, parcel_buffer& buffer,
        std::size_t const parcel_count, std::size_t const num_thread)
    {
        std::vector<serialization::serialization_chunk> chunks(
        ...
        std::vector<parcelset::parcel> parcels =
            decode_message_with_chunks_zero_copy(HPX_MOVE(add_received_data),
                buffer, parcel_count, chunks, num_thread);
