...
double run_search_n_benchmark(int test_count, ExPolicy policy,
    std::vector<int>& c, int needle_count)
{
    std::uint64_t time = 0;
    std::ptrdiff_t acc = 0;
    std::size_t n = c.size();
    ...
    int test_count, std::vector<int>& c, int needle_count)
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        std::uint64_t elapsed = hpx::chrono::high_resolution_clock::now();
        auto result = std::search_n(std::begin(c), std::end(c), needle_count, 2);
        time += hpx::chrono::high_resolution_clock::now() - elapsed;
        acc += std::distance(std::begin(c), result);
