    ...
    int test_count, std::vector<int>& c, std::vector<int>& needle)
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        std::uint64_t elapsed = hpx::chrono::high_resolution_clock::now();
        auto result = std::search(std::begin(c), std::end(c),
            std::begin(needle), std::end(needle));
        time += hpx::chrono::high_resolution_clock::now() - elapsed;
        acc += std::distance(std::begin(c), result);
...
void run_benchmark(std::size_t vector_size, int test_count, int needle_count)
{
    ...
    double time_seq = run_search_benchmark(
        test_count, hpx::execution::seq, c, needle);
    double time_par = run_search_benchmark(
        test_count, hpx::execution::par, c, needle);
    double time_simd = run_search_benchmark(
        test_count, hpx::execution::simd, c, needle);
    double time_par_simd = run_search_benchmark(
        test_count, hpx::execution::par_simd, c, needle);
    ...
    hpx::util::format_to(std::cout, fmt, "par_simd", time_par_simd) << std::endl;
    std::cout << "----------------------------------------------" << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    desc_commandline.add_options()(
        "vector_size", value<std::size_t>()->default_value(10000000),
        "size of vector (default: 10000000)")(
        "needle_count", value<int>()->default_value(8),
        "length of the subsequence to search for (default: 8)")(
